...
public class PokemonService implements PokemonServiceI {
	...
	public void fillPokemonInfo(Pokemon pokemon) {
		StringBuffer response = new StringBuffer();
		try {
			String urlString = String.format("https://pokeapi.co/api/v2/pokemon/%d/", pokemon.getId());
			URL url = new URL(urlString); // MalformedURLException
			HttpURLConnection con = (HttpURLConnection) url.openConnection(); // IOException
			con.addRequestProperty("User-Agent", USER_AGENT);
			con.setRequestMethod("GET"); // ProtocolException
		...
		if (response.length() != 0) {
			try {
				...
				final JSONObject sprites = obj.getJSONObject("sprites");
				final String imgUrl = sprites.getString("front_default");
